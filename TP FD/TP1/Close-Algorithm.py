...
def close(minsupport):
    ...
    assocs = associations(minsupport,Gen1)
    print(assocs)   #generator -> closed itemsets - generator
        ...
        if verif == False:
            ...
            while True:
                ...
                if verif == True:
                    ...
                    compteur=compteur+1
                    Gen2=Gen3
    return Gen1, assocs, minsupport

minsupport=float(input("enter Minsup: "))
close(minsupport)

def get_minsup():
    minsup = minsup_input.get()
    Gen1, assocs, minsupport = close(float(minsup))
    output_textbox.delete('1.0', tk.END)
    output_textbox.insert(tk.END, "GEN1: " + str(Gen1) + "\n Les associations: " + str(assocs) + "\n MinSupport : " + str(minsupport))

window = tk.Tk()
window.title("MinSup Calculator")
window.geometry("400x300")

minsup_label = tk.Label(window, text="Enter MinSup:")
minsup_label.pack(pady=10)
minsup_input = tk.Entry(window)
minsup_input.pack()

submit_button = tk.Button(window, text="Submit", command=get_minsup)
submit_button.pack(pady=10)

output_label = tk.Label(window, text="Outputs:")
